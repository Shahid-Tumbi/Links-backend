...
import { CuratorDataFormat } from './data.formatter';
...
import {
	...
	ILogoutData,
	IChangePassword,
} from './curator.interface';
import { FollowModel, FollowRequestModel, User, UserDetailModel } from './curator.model';
import { VerifyOtpData } from './curator.swagger';
...
class UserService {
	...
	async register(
		...
		try {
			...
			return { profile: CuratorDataFormat(auth, profile), token, otpNumber };
		} catch (error) {
	...
	async login(req?: App.Request) {
		...
		return { profile: CuratorDataFormat(result,user), token };
	}
