"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.apiService = void 0;
const utils_1 = require("../../utils");
const api_model_1 = require("./api.model");
const api_constants_1 = require("./api.constants");
class ApiService {
    config(req, user) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const configData = yield api_model_1.ConfigModel.findOneAndUpdate({}, api_constants_1.AppConfig, { upsert: true, new: true });
                const _a = configData._doc, { __v, _id } = _a, cleanedConfigData = __rest(_a, ["__v", "_id"]);
                return cleanedConfigData;
            }
            catch (error) {
                utils_1.Console.error('Error in  service', error);
                return Promise.reject(new utils_1.ResponseError(422, error));
            }
        });
    }
}
exports.apiService = new ApiService();
